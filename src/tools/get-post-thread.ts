...
export const getPostThreadTool: Tool = {
	...
	description: "Get a complete conversation thread including the post, its parents, and replies",
	inputSchema: {
		type: "object",
		properties: {
			uri: {
				type: "string",
				description: "The URI of the post to get the thread for (e.g. at://did:plc:abcdef/app.bsky.feed.post/12345)",
			},
			depth: {
				type: "number",
				description: "The levels of reply depth to fetch below the post (default: 0, max: 1000). Set to a higher number like 10-20 to see all replies.",
			},
			parentHeight: {
				type: "number",
				description: "The number of parent posts to include above the post (default: 0, max: 1000). Set to a higher number like 10-20 to see the full conversation context.",
			},
		},
		required: ["uri"],
	},
...
export async function handleGetPostThread(
	...
	try {
		...
		const formattedThread = formatThreadForDisplay(response);
...
function formatThreadForDisplay(response: any): string {
	try {
...
function formatThreadNode(node: any, level: number): string {
	if (!node) return "";
