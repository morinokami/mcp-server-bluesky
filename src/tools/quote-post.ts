	...
	inputSchema: {
		...
		properties: {
			...
			uri: {
				type: "string",
				description:
					"The URI of the post to quote (e.g. at://did:plc:abcdef/app.bsky.feed.post/12345)",
			},
			cid: {
				type: "string",
				description:
					"The CID of the post to quote (usually from a previous search or get-post-thread call)",
			},
		},
...
export async function handleQuotePost(
	...
	try {
				...
				content: [
					{
						type: "text",
						text:
							`Quote post is too long. Please remove approximately ${overage} character${overage !== 1 ? "s" : ""}.\n` +
				...
				content: [
					{
						type: "text",
						text:
							`Quote post successful! Reference: ${response.uri}\n` +
		...
		return {
			content: [
				{ type: "text", text: "Quote post failed due to an unknown error" },
			],
		};
	}
}
