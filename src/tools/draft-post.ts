...
export const createDraftTool: Tool = {
  ...
  description: "Create a draft post of any length that will be split into a thread when published",
  inputSchema: {
    type: "object",
    properties: {
      content: {
        type: "string",
        description: "The content of your draft post, can be of any length and will be split appropriately into multiple posts",
      },
      title: {
        type: "string",
        description: "Optional title for the draft (for your reference only, won't be published)",
      },
    },
    required: ["content"],
  },
...
export const publishDraftTool: Tool = {
  ...
  description: "Publish a draft post as a thread",
  inputSchema: {
    type: "object",
    properties: {
      draftId: {
...
export async function handleCreateDraft(
  ...
  try {
    ...
    const preview = draft.chunks?.map((chunk, index) => 
...
export async function handleListDrafts(
  ...
  try {
    ...
    if (drafts.size === 0) {
      return {
        content: [{ type: "text", text: "No drafts available. Create a draft first with bluesky_create_draft." }],
      };
    }
  ...
  for (const paragraph of paragraphs) {
    ...
    if (getGraphemeLength(paragraph) > MAX_POST_LENGTH) {
      ...
      for (const sentence of sentences) {
        ...
        if (getGraphemeLength(currentChunk + (currentChunk ? " " : "") + sentence) > MAX_POST_LENGTH) {
          ...
          if (getGraphemeLength(sentence) > MAX_POST_LENGTH) {
            ...
            for (const word of words) {
              if (getGraphemeLength(wordChunk + (wordChunk ? " " : "") + word) > MAX_POST_LENGTH) {
                // If we already have a current chunk, add the word chunk to it if possible
                if (currentChunk && getGraphemeLength(currentChunk + " " + wordChunk) <= MAX_POST_LENGTH) {
    ...
    } else if (getGraphemeLength(currentChunk + (currentChunk ? "\n\n" : "") + paragraph) > MAX_POST_LENGTH) {
...
async function getCidFromUri(agent: AtpAgent, uri: string): Promise<string> {
  try {
    ...
    throw new Error(`Failed to get CID for URI ${uri}: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
